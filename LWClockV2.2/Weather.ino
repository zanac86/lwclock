String GetWeather()
{
    ...
    {
        Serial.print(F("No connection to weather server (No Wi-Fi) "));
    ...
    {
        Serial.print(F("Connection weather server failed"));
        Serial.println(W_URL);
        weatherString = "*" + oldWeather;
        return weatherString;
    }
    ...
    weatherString += hum[lang];
    weatherString += String(humidity);
    weatherString += pres[lang];
    weatherString += String(pressure / 1.3332239, 0);
    ...
    {
        weatherString += windir_table[lang][0];
    }
    else
        if (deg > 67.5 && deg <= 112.5)
        {
            weatherString += windir_table[lang][1];
        }
        else
            if (deg > 112.5 && deg <= 157.5)
            {
                weatherString += windir_table[lang][2];
            }
            else
                if (deg > 157.5 && deg <= 202.5)
                {
                    weatherString += windir_table[lang][3];
                }
                else
                    if (deg > 202.5 && deg <= 247.5)
                    {
                        weatherString += windir_table[lang][4];
                    }
                    else
                        if (deg > 247.5 && deg <= 292.5)
                        {
                            weatherString += windir_table[lang][5];
                        }
                        else
                            if (deg > 292.5 && deg <= 337.5)
                            {
                                weatherString += windir_table[lang][6];
                            }
                            else
                            {
                                weatherString += windir_table[lang][7];
                            }
    uint8_t clouds = root["clouds"]["all"];
    weatherString += cloudstxt[lang] + String(clouds) + "% ";
    Serial.print(F("Get weather: "));
...
String GetWeatherForecast()
{
    ...
    weatherString += String(humidity);
    weatherString += pres[lang];
    weatherString += String(pressure, 0);
    weatherString += wind[lang];
    ...
    weatherString += windsp[lang];
    Serial.print(F("Get weather forecast: "));
    Serial.println(weatherString);
    oldWeather =  weatherString;
    return weatherString;
}
...
String GetSea()
{
    ...
    if (tempSea != "")
    {
        return  String(tempersea[lang] + tempSea + "\xC2С");
    }
    else
    {
        return "";
