...
void setup()
{
    ...
    char host[16];
    FS_init();
    Serial.println(F("Start FS"));   //Run FS
    loadConfig();
    Serial.println(F("FileConfig"));
    Display_init();
    Serial.println(F("Start screen"));
    if (myWIFI.begin(ssid.c_str(), password.c_str(), "????", ssidAP.c_str(), passwordAP.c_str(), apIP.c_str()))
    {
        Serial.println(F("Start WIFI"));  //Run WIFI
    }
    Serial.println(myWIFI.getDevStatusIP());
    Time_init();
    Serial.println(F("Start Time"));  // Get time from NTP
    server_httpinit();
    Serial.println(F("Start HTTPServer"));   //Run HTTP server
    if (WiFi.status() != WL_CONNECTED)
    {
        String sText = " " + WiFi.softAPIP().toString();
        sText.toCharArray(buf, 256);
        P.displayScroll(buf, PA_LEFT, PA_SCROLL_LEFT, 20);
        Serial.println(sText);
    }
    else
    {
        String sText = " " +  WiFi.localIP().toString();
        sText.toCharArray(buf, 256);
        P.displayScroll(buf, PA_LEFT, PA_SCROLL_LEFT, 20);
        Serial.println(sText);
    }
...
void loop()
{
    ...
    {
        if (modeShow > maxModeShow)
        {
            modeShow = 1;
        }
        if (oldModeShow != modeShow)
        {
            Serial.println("modeShow: " + String(modeShow));    //****************************
            oldModeShow = modeShow;
            displayClockCount = millis();
            displayClockFirst = true;
        }
        ...
        if (modeShow == 11)   //Show Temp, hum, press
        {
            ...
            if (isLedTHP && compTimeInt(thpFrom, thpTo, nowtime))
            {
                if (millis() - lastTimePHT > PER_GET_THP)   //чтобы не грузить
                {
                    ...
                    tempUP = getTempDHT() + "\xC3";
                    humUp = getHumDHT() + "%";
#endif
#if USE_BME280 == true
                    //strTHP = onboard[lang] + getTempBME280() + hum[lang] + getHumBME280() + pres[lang] + getPressBME280() + "mm";
                    tempUP = getTempBME280(dataCorrect, 1) + "\xC3";
                    humUp = getHumBME280(dataCorrect, 0) + "%";
                    pressUP = getPressBME280(0) + "\xC4";
#endif
                ...
                if (modeShowUP == 1)
                {
                    modeShowUP += showTextUP(tempUP, 5 * speedTicker, 1750, PA_SCROLL_UP, PA_SCROLL_UP_LEFT, false);
                }
                //if (modeShowUP==3) modeShowUP +=showTextUP(humIn[lang], 5*speedTicker, 500, PA_SCROLL_UP, PA_SCROLL_UP, false);
                if (modeShowUP == 2)
                {
                    modeShowUP += showTextUP(humUp, 5 * speedTicker, 1750, PA_SCROLL_UP, PA_SCROLL_UP_RIGHT, true);
                }
                ...
                if (modeShowUP == 2)
                {
                    modeShowUP += showTextUP(humUp, 5 * speedTicker, 1750, PA_SCROLL_UP, PA_SCROLL_UP_RIGHT, false);
                }
                //if (modeShowUP==5) modeShowUP +=showTextUP(presIn[lang], 5*speedTicker, 500, PA_SCROLL_UP, PA_SCROLL_UP, false);
                if (modeShowUP == 3)
                {
                    modeShowUP += showTextUP(pressUP, 5 * speedTicker, 1750, PA_SCROLL_UP, PA_SCROLL_UP_RIGHT, true);
                }
                //showText(strTHP, PA_LEFT, 5*speedTicker, 0, PA_SCROLL_LEFT, PA_SCROLL_LEFT);
#endif
            }
