...
void handle_ConfigJSON()
{
    ...
    json["isLedWeather"] = (isLedWeather ? "checked" : "");
    json["isLedForecast"] = (isLedForecast ? "checked" : "");
...
//    SSDP_Name = HTTP.arg("ssdp").c_str();
    myWIFI.setConfigWIFI(ssid.c_str(), password.c_str(), "????", ssidAP.c_str(), passwordAP.c_str());
    //saveConfig();
...
void handle_LedSetup()
{
    ...
    Serial.print("speedTicker ");
    Serial.println(speedTicker);
    Serial.print("brightd ");
    Serial.println(brightd);
    Serial.print("brightn ");
    Serial.println(brightn);
    Serial.print("dmodefrom ");
    Serial.println(dmodefrom);
    Serial.print("dmodeto ");
    Serial.println(dmodeto);
    HTTP.send(200, "text/plain", "OK");
}
...
void handle_LedOption()
{
    ...
    weathFrom = HTTP.arg("weathFrom").toFloat();
    weathTo = HTTP.arg("weathTo").toFloat();
    fcastFrom = HTTP.arg("fcastFrom").toFloat();
    fcastTo = HTTP.arg("fcastTo").toFloat();
    clockFrom = HTTP.arg("clockFrom").toFloat();
    clockTo = HTTP.arg("clockTo").toFloat();
    dateFrom = HTTP.arg("dateFrom").toFloat();
    dateTo = HTTP.arg("dateTo").toFloat();
    seaFrom = HTTP.arg("seaFrom").toFloat();
    seaTo = HTTP.arg("seaTo").toFloat();
#if (USE_BME280 == true || USE_DHT == true)
    HTTP.arg("isLedTHP").toInt() == 1 ? isLedTHP = true : isLedTHP = false;
    thpFrom = HTTP.arg("thpFrom").toFloat();
    thpTo = HTTP.arg("thpTo").toFloat();
#else
    isLedTHP = false;
    thpFrom = 25;
    thpTo = 25;
...
void handle_Texts()
{
    ...
    Serial.println(strText0);
    Serial.println(strText1);
    Serial.println(strText2);
    Serial.println(strText3);
    Serial.print("isTxtOn0 ");
    Serial.println(isTxtOn0);
    Serial.println(txtFrom0);
    Serial.println(txtTo0);
    Serial.println(isCrLine0);
    Serial.print("isTxtOn1 ");
    Serial.println(isTxtOn1);
    Serial.println(txtFrom1);
    Serial.println(txtTo1);
    Serial.println(isCrLine1);
    Serial.print("isTxtOn2 ");
    Serial.println(isTxtOn2);
    Serial.println(txtFrom2);
    Serial.println(txtTo2);
    Serial.println(isCrLine2);
    Serial.print("isTxtOn3 ");
    Serial.println(isTxtOn3);
    Serial.println(txtFrom3);
    Serial.println(txtTo3);
    Serial.println(isCrLine3);
    HTTP.send(200, "text/plain", "OK");
    P.displaySuspend(false);
}
...
void handle_Language()
{
    if (HTTP.arg("lang") == "RU" && lang != 0)
    {
        ...
        if (HTTP.arg("lang") == "BG" && lang != 1)
        {
            lang = 1;
        }
        else
            if (HTTP.arg("lang") == "EN" && lang != 2)
            {
                lang = 2;
            }
            else
    ...
    Serial.println(HTTP.arg("lang").c_str());
    saveConfig();
    if (isLedWeather)
    {
        strWeather = GetWeather();
